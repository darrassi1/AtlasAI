...
class AgentTemplate:
    ...
    def parse_answer(self, response: str) -> dict | bool:
        """
        This method tries to parse the response from the model to a dict based on the prompt structure.
        ...
        try:
            ...
            final_json1 = json.loads(final_json, strict=False)
        ...
        for field in required_fields:
            if field not in final_json1:
                logging.warning(f"Missing field {field} in the response.")
                return False

        return {field: final_json1[field] for field in required_fields}
